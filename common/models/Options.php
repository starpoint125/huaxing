...
class Options extends \yii\db\ActiveRecord
{
    ...
    public static function getIndexBannerPic($type_name){
        $model = Options::find()->Where(['like','name',trim($type_name)])->limit(5)->orderBy('sort desc')->all();
        if ($model == null) {
            throw new NotFoundHttpException("None banner type named $name");
        }
        $new_arr = [];
        foreach ($model as $key => $value) {
            // echo "<pre>";var_export($value);exit;
            $json_decode             = json_decode($value['value']);
            $new_arr[$key]['ad']     = $json_decode->ad;
            $new_arr[$key]['url']    = $json_decode->link;
            $new_arr[$key]['target'] = $json_decode->target;
            $new_arr[$key]['desc']   = $json_decode->desc;
        }
        return $new_arr;
    }
